...
public class VideoPlayer extends FrameLayout
        implements VVideoPlayer,
        VTextureView.SurfaceTextureListener {
    ...
    private VTextureView mITextureView;
...
//        }
    ...
    private void initTextureView() {
        if (mITextureView == null) {
            mITextureView = new VTextureView(mContext);
            mITextureView.setSurfaceTextureListener(this);
        }
    }

    private void addTextureView() {
        mContainer.removeView(mITextureView);
        LayoutParams params = new LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.MATCH_PARENT,
                Gravity.CENTER);
        mContainer.addView(mITextureView, 0, params);
    }
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surfaceTexture, int width, int height) {
        if (mSurfaceTexture == null) {
            ...
            mITextureView.setSurfaceTexture(mSurfaceTexture);
        }
    }
    ...
    private void openMediaPlayer() {
        ...
        mMediaPlayer.setHardwareDecoder(true);
        mMediaPlayer.setBufferStrategy(3);
    ...
    private NELivePlayer.OnPreparedListener mOnPreparedListener
            = new NELivePlayer.OnPreparedListener() {
        @Override
...
//            LogUtil.d("onCompletion ——> STATE_COMPLETED");
    ...
    private NELivePlayer.OnErrorListener mOnErrorListener
            = new NELivePlayer.OnErrorListener() {
        @Override
...
//                LogUtil.d("onError ——> STATE_ERROR ———— what：" + what + ", extra: " + extra);
    ...
    private NELivePlayer.OnInfoListener mOnInfoListener
            = new NELivePlayer.OnInfoListener() {
        @Override
        public boolean onInfo(NELivePlayer mp, int what, int extra) {
            if (what == NEType.NELP_FIRST_VIDEO_RENDERED) {
                // 播放器开始渲染
                mCurrentState = STATE_PLAYING;
                mController.onPlayStateChanged(mCurrentState);
            } else if (what == NEType.NELP_BUFFERING_START) {
...
//            else if (what == NELivePlayer.MEDIA_INFO_VIDEO_ROTATION_CHANGED) {
//                // 视频旋转了extra度，需要恢复
//                if (mTextureView != null) {
//                    mTextureView.setRotation(extra);
////                    LogUtil.d("视频旋转角度：" + extra);
//                }
//            } else if (what == NELivePlayer.MEDIA_INFO_NOT_SEEKABLE) {
...
//            }
    ...
    private NELivePlayer.OnBufferingUpdateListener mOnBufferingUpdateListener
            = new NELivePlayer.OnBufferingUpdateListener() {
        @Override
